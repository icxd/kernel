...
class String {
public:
  String(const char *string) : m_string(string), m_size(strlen(string)) {}
  String(const char *string, usz size) : m_string(string), m_size(size) {}

  [[nodiscard]] String substring(usz start, usz end) const;
  [[nodiscard]] String substring(usz start) const { return substring(start, size()); }
  ...
  [[nodiscard]] char operator[](usz index) const { return at(index); }
